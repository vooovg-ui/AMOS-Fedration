...
## تاريخ آخر تعديل: 2026-08-27 (‏`WI-013` — حرسُ نجاةِ الحالةِ كانَ ينسِبُ نقصَ البيئةِ إلى إدامةِ الحالةِ فيُنتِجُ حكمًا أحمرَ كاذبًا؛ فصارَ شرطُ القياسِ الحيِّ مقيسًا من مُخرَجِ المِسبارِ (`DISC-019` · `W-058`) · وقبلَه `WI-012` — انحرافُ وثائقِ الحالةِ صارَ مقيسًا (صِفرٌ) · وتعارُضُ `DISC-012` يحبِسُ `WI-006`…`WI-013` مفتوحةً)
