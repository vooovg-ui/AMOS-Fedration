...
class InMemoryDataPipeline:
    ...
    `training/main.py:36` يُهيِّئُ هذا الصنفَ على مستوى الوحدةِ، وأنَّ
    نقطتَي `POST /v1/datasets` و`GET /v1/datasets/{id}` تقرآنِ منه وتكتبانِ فيه.
    فمجموعاتُ البياناتِ الـمُنشأةُ بـ201 **تتبخّرُ عندَ إعادةِ التشغيلِ** ولا يراها
    عاملٌ آخرُ في العمليّةِ نفسِها لو تعدَّدَتِ العمليّات. ولا نظيرَ دائمًا لها في
    `common/database.py` (لا جدولَ `datasets`) — فالإدامةُ عملُ T4/E4 لا عملُ إعلانٍ،
    وملكيّةُ القرارِ (أيُّ بياناتٍ سجلٌّ سياديٌّ يُحفَظُ؟) مقيَّدةٌ في Q-39.
    """
