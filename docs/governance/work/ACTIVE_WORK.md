...
## تاريخ آخر تعديل: 2026-08-27 (‏`WI-012` — انحرافُ وثائقِ الحالةِ عن السجلِّ صارَ مقيسًا (9 و8 قيودٍ تأخُّرًا → صِفرٌ) · وقبلَه `WI-011` — حالةُ القراراتِ السياديّةِ صارت مقيسةً (`A-2` · `A-3` باقيانِ `PENDING`) · وقبلَه `WI-010` — الجردُ المُعلَنُ صارَ مقيسًا · وتعارُضُ `DISC-012` يحبِسُ `WI-006`…`WI-009` مفتوحةً)
