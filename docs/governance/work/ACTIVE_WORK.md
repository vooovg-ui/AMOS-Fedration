...
## تاريخ آخر تعديل: 2026-09-05 (**`WI-048`** — فُتِحَ `READY` على `DISC-056`: قارئُ حكمِ CI لا يُثبِّتُ محاولةً ولا يكتُبُ أيَّها قرأَ، فالتشغيلُ **94** ([33818260611](https://github.com/xoos-beep/AMOS-Fedration/actions/runs/33818260611)) على عقدةِ `283bf1b` قُرِئَ `READABLE · 13/13` في محاولتِه الأولى ثمَّ `UNREADABLE · 0/10` في الثانيةِ **بالرقمِ نفسِه** · ومسارَاه مقفولانِ بـ`WI-024` و`WI-032` فالانتظارُ مُعلَنٌ · قُيِّدَ بـ`W-136`) · وقبلَه 2026-09-04 (**`WI-047`** — قُرِئَ حكمُ تشغيلِ **91** (`33798360474`) على عقدةِ `575b911` ⇒ `completed success` · **13/13** · `READABLE` (و`ci_verdict_readability.py --from-json` ⇒ خروجٌ 0) فنُقِلَ البندُ إلى `IN_REVIEW` بلا `VERIFIED` — `A-2` `PENDING` · `DISC-027` · `RK-020`) · وقبلَه 2026-09-03 (**`WI-043`** — فُتِحَ ونُفِّذَ: `DISC-051` صارَ سقّاطةً مقيسةً — **11 مُولِّدًا · 4 مربوطةٌ · 7 بلا رباطٍ** — فمُولِّدٌ يُولَدُ خارجَ الحرسِ يُسقِطُ الوجهَ، و23 فحصًا تُثبِتُ الفرقَ · وأوّلُ نسخةٍ عَدَّت نفسَها مُولِّدًا فأُصلِحَ السببُ نحويًّا لا العَرَضُ) · وقبلَه 2026-09-03 (`WI-039` — قيدُ `W-126`: رُقِّيَت ثلاثةٌ وعشرونَ صفًّا إلى إعلانٍ مُهيكَلٍ فانخفضَت سقّاطةُ «مسارٍ في نثرٍ» **25 ← 2**، وقُيِّدَ `DISC-048` وأُصلِحَ موضعُه في الأداةِ، و**فُتِحَ `WI-040`** (`READY`) لمسحِ سائرِ قارئاتِ الجداولِ) · وقبلَه 2026-09-03 (`WI-039` — `RESERVED` ← `IN_PROGRESS`: نزلَت طبقةُ **الإعلانِ المُهيكَلِ** وسقّاطتُها في `open_record_accountability.py` وحرسُها في `tests/governance/test_w124_anchor_declaration.py` · **وقلبُ التصنيفِ نفسِه محجوبٌ**: حرسُ الأداةِ `tests/governance/test_w059_open_record_accountability.py` مُدَّعًى لـ`WI-033` وحالتُه `IN_REVIEW` فلا يُمَسُّ · قيدُ `W-125`) · وقبلَه 2026-09-03 (`WI-039` — حُجِزَ لـ`DISC-021`: مُصنِّفُ المِرساةِ يقرأُ مسارًا في نثرٍ حرسًا قائمًا · **والحاجزُ المكتوبُ فيه أُعيدَ قياسُه فلم يَعُدْ قائمًا**: `WI-014` مُغلَقٌ فالمسارُ غيرُ مُدَّعًى · وحكمُ تشغيلِ 81 على عقدةِ `5f805e1` قُرِئَ أخضرَ **13/13** `READABLE`) · وقبلَه 2026-09-03 (`WI-038` — حكمُ تشغيلِ 80 قُرِئَ أخضرَ 13/13 `READABLE` بـ`W-123` فنُقِلَ البندُ إلى `IN_REVIEW` بلا `VERIFIED`) · وقبلَه 2026-09-03 (`WI-038` — حكمُ CI 79 الأحمرُ أصلحَ جذرَ الحرسِ بـ`W-122`: الإعفاءُ البيئيُّ يُقرأُ من خرجِ الجمعِ بأسماءِ الحزمِ الغائبةِ · `DISC-047`) · وقبلَه 2026-09-03 (`WI-038` — نزلَ حرسُه بـ`W-121`: عددُ جمعِ الحزمةِ صارَ مقيسًا حيًّا يُقارَنُ بالمكتوبِ، فسقطَ الرقمُ الميّتُ (‏1749 مقابلَ 2352 · و1383 مقابلَ 1428) وصُحِّحَت أربعةُ مواضعَ بقياسٍ طازجٍ · 13 فحصًا · البندُ `IN_PROGRESS` بلا `VERIFIED`) · وقبلَه 2026-09-03 (`WI-038` — حُجِزَ لـ`DISC-046`: عددُ جمعِ الحزمةِ يصيرُ رقمًا يُقاسُ بأمرٍ رخيصٍ لا يُنقَلُ) · وقبلَه 2026-09-03 (`WI-037` — حكمٌ أخضرُ مقروءٌ **13/13** على عقدةِ `c5d3798` (‏تشغيلٌ **76** · `READABLE`) فصارَ البندُ `IN_REVIEW` بلا `VERIFIED` — فخمسةَ عشرَ بندًا في يدِ المراجعِ) · وقبلَه 2026-09-03 (`WI-037` — حُجِزَ: `DISC-042` المستحقُّ 2026-09-09 — خليّةُ المساراتِ تُشطَرُ فيُقرأُ نثرُها دعوى، والمقيسُ اليومَ **8** أجزاءٍ غيرِ مساريّةٍ في 4 بنودٍ؛ وحاجزُ الملكيّةِ المكتوبُ فيه أُعيدَ قياسُه فلم يَعُدْ قائمًا) · وقبلَه 2026-09-03 (`WI-036` — حكمٌ أخضرُ مقروءٌ **13/13** على عقدةِ `c817ada` (‏تشغيلٌ **73** · `READABLE`) فصارَ البندُ `IN_REVIEW` بلا `VERIFIED` — فأربعةَ عشرَ بندًا في يدِ المراجعِ) · وقبلَه 2026-09-03 (`WI-035` — **حكمٌ أخضرُ مقروءٌ 13/13** على عقدةِ `2c59165` (‏تشغيلٌ **71** · `33697957662` · `READABLE`) فصارَ البندُ `IN_REVIEW` بلا `VERIFIED` — فثلاثةَ عشرَ بندًا في يدِ المراجعِ؛ و`WI-036` حُجِزَ: `DISC-045` — الرقمُ 9 في § 10 مكتوبٌ **مرّتَينِ** فـ**7 إحالاتٍ** إلى «الحدِّ 9» لا تُحَلُّ) · وقبلَه 2026-09-02 (`WI-035` — حكمٌ أحمرُ **ثالثٌ** مقروءٌ على عقدةِ `66c217b` (‏تشغيلُ `33621763476` · 11/13): **العقدةُ التي أعلنَت الرقمَ أسقطَت سقّاطتَها بنفسِها** لأنَّ إزاحتَها محلّيّةٌ، فصُحِّحَ المُعلَنُ **134 ← 135** وأُضيفَ وجهٌ سابقٌ للدفعِ يُلزِمُ ما بعدَ `baseline_node` بإزاحةٍ عالميّةٍ · `DISC-044`) · وقبلَه 2026-09-02 (`WI-035` — حكمٌ أحمرُ ثانٍ مقروءٌ من تعليقاتِ الفحصِ: مصدرُ القياسِ كانَ يُوحِّدُ الإزاحاتِ فقاسَ **0** والحقيقةُ **134**، فصُحِّحَ المُعلَنُ وحُذِفَت الدعوى الكاذبةُ) · وقبلَه 2026-09-02 (`WI-035` — حكمٌ أحمرُ مقروءٌ على عقدةِ `89efcdec` سببُه خطأُ منهجٍ في الرقمِ المُعلَنِ، فصُحِّحَ **3 ← 4** ولم تُخفَّفْ بوّابةٌ) · وقبلَه 2026-09-02 (`WI-035` — صدقُ طوابعِ الالتزامِ صارَ رقمًا مقيسًا محروسًا من النموِّ: **353 التزامًا · 0 بإزاحةٍ محلّيّةٍ · 3 مجموعاتٍ مكرَّرةٍ تضمُّ 10 · و3 مقلوبةٌ** · و`WI-034` صارَ `IN_REVIEW` بحكمٍ أخضرَ مقروءٍ **13/13** على عقدةِ `8693658`) · وقبلَه 2026-09-02 (`WI-034` — فخُّ منطقةِ زمنِ الالتزامِ صارَ محروسًا: الحرسُ يُثبِتُ الفخَّ ثمَّ يقيسُ زوالَه · و`WI-033` صارَ `IN_REVIEW` بحكمٍ أخضرَ مقروءٍ **13/13** على عقدةِ `3c5025d`) · وقبلَه 2026-09-02 (`WI-033` — حكمٌ أخضرُ مقروءٌ **13/13** على عقدةِ `ceef58e` فصارَ `IN_REVIEW` · وما بقيَ حرًّا من الدَّينِ موضعانِ تحتَ `tools/crown/` حبسُهما قرارُ مالكٍ لا عملٌ) · وقبلَه 2026-09-02 (`WI-032` — حكمٌ أخضرُ مقروءٌ **13/13** على عقدةِ `9c27533` فصارَ `IN_REVIEW` · و`WI-033` حُجِزَ ونُفِّذَ: 9 مواضعَ حرّةٍ باقيةٍ تحتَ `tests/`) · وقبلَه 2026-09-02 (`WI-032` — نُفِّذَ الخفضُ: 34 موضعًا رُحِّلَت، فصارَ دَينُ `tests` **19** ومن صنفِ «بلا وجهٍ» **12**) · وقبلَه 2026-09-02 (`WI-032` — حُجِزَ: أوّلُ خفضٍ فعليٍّ لدَينِ `DISC-041` في 34 موضعًا حرًّا) · وقبلَه 2026-09-02 (`WI-031` — حكمانِ أخضرانِ مقروآنِ 13/13 فصارَ `IN_REVIEW` · و`DISC-041` مِرساتُه صارَت حرسًا قائمًا) · وقبلَه 2026-09-02 (`WI-031` — نزلَ الحرسُ وصارَ `IN_PROGRESS`: الدَّينُ صارَ سقّاطتَينِ لا تعلوانِ) · وقبلَه 2026-09-02 (`WI-031` — حُجِزَ: جذرُ القياسِ يُعرَفُ بعلامةٍ أو بعلمٍ لا بعُمقٍ مكتوبٍ · و`DISC-041` قُيِّدَ بأرقامٍ تُعادُ: **25 و53 موضعًا · منها 2 و37 بلا وجهٍ يُمرِّرُ جذرًا صريحًا**) · وقبلَه 2026-09-02 (`WI-030` — نُقِلَ إلى `IN_REVIEW` بعدَ حكمٍ أخضرَ مقروءٍ **13/13** على عقدةِ `48082c5`، والفحصُ الجديدُ نُفِّذَ في CI لا محلّيًّا وحدَه) · وقبلَه 2026-09-02 (`WI-030` — نزلَ وجهُ `--check` وحرسُه فصارَ «الأثرُ المُولَّدُ مدفوعٌ» مقيسًا **قبلَ** الدفعِ لا في CI وحدَها: **6 فحوصٍ مرَّت · مواضعُ القياسِ 27 منها 0 غيرُ مُرساةٍ · والسقّاطةُ ثابتةٌ عندَ 63 بعدَ إصلاحِ عَطبٍ أحدثَه القياسُ في نفسِه**) · وقبلَه 2026-09-01 (`WI-030` — حُجِزَ: «الأثرُ المُولَّدُ مدفوعٌ» يصيرُ مقيسًا قبلَ الدفعِ لا في CI وحدَها · و`WI-029` نُقِلَ إلى `IN_REVIEW` بعدَ حكمٍ أخضرَ مقروءٍ 13/13) · وقبلَه 2026-09-01 (`WI-029` — الحرسُ نزلَ فصارَ النمطُ مقيسًا في كلِّ دفعةٍ، والموضعُ الوحيدُ غيرُ المُرسى أُرسِيَ بعلمٍ صريحٍ: **24 موضعًا · 19 قصدًا · 5 مؤقَّتةً · 5 مُرساةٌ · 0 غيرُ مُرساةٍ**) · وقبلَه 2026-08-30 (`WI-021` — ماسحٌ واحدٌ مرجعيٌّ لبوّابةِ 6، ومحلُّ قياسٍ صريحٌ، وحالةٌ لا تُكتَبُ مرّتَينِ متناقضتَينِ) · وقبلَه 2026-08-29 (`WI-019` — الحكمُ الأحمرُ صارَ مقروءًا ثمَّ صارَ أخضرَ: سببُه سطرُ اختبارٍ لا حرسٌ ضعيفٌ، فأُصلِحَ الفتيلُ ولم يُخفَّفِ الماسحُ)
...
| WI-023 | tooling-gates | T0 / T0.4ب (قابليّةُ القياسِ — دعوى «يُشغَّلُ في كلِّ دفعةٍ» تُقاسُ قبلَ أن تُقبَلَ سندًا) | Driving H | المالك (`A-2` · غيرُ مستقلٍّ) | IN_REVIEW | tools/governance/guard_enforcement_closure.py · tests/governance/test_w077_guard_enforcement_closure.py · .github/workflows/ci.yml · PROJECT_STATE.md · docs/PROJECT_HANDBOOK.md | 2026-08-31 | 2026-09-07 | — | **المراجعةُ بيدِ المالكِ**: `VERIFIED` ثمَّ `CLOSED` قرارُه لا قرارُ منفِّذٍ — والمراجعةُ **ليست مستقلَّةً** بنصِّ `A-2` (`DISC-027` · `RK-020`) | W-078 · حكمُ عقدةِ `4764a12` مقروءٌ **13/13 · READABLE** (تشغيلُ 33391458311) |
...
العائق: لا عائقَ فنيًّا. والمسارانِ مُدَّعيانِ لبندَينِ في المراجعةِ (`WI-024` · `WI-032`) فلا يُحجَزانِ قبلَ تحرُّرِهما — والانتظارُ مُعلَنٌ لا مطويٌّ. وتحرُّرُهما موقوفٌ على `A-2` (`PENDING`) وهو قرارُ مالكٍ.
...
حدُّ البندِ — مُعلَنٌ لا مطويٌّ:
  ...
  - **ولا `VERIFIED` بيدِ الكاتبِ** (§ 4.3 · `A-2` `PENDING` · `DISC-027` · `RK-020`)
...
العائق: المسارانِ مُدَّعيانِ لبندٍ `IN_REVIEW` (`WI-038`) — والمراجعةُ بيدِ المالكِ (`A-2` `PENDING`)
...
العائق: —
الخطوةُ التالية: مراجعةٌ ثمَّ `VERIFIED` — بيدِ المراجعِ وحدَه، و`A-2` `PENDING` (`DISC-027` · `RK-020`)
...
بدأ: 2026-09-02        ينتهي الحجز: 2026-09-09
العائق: —
الخطوةُ التالية: مراجعةٌ — و`A-2` `PENDING` والمراجعُ غيرُ مستقلٍّ (`DISC-027`)، فلا يُكتَبُ `VERIFIED` هنا بحالٍ
...
المالك: Driving H            المراجع: — (‏النطاقُ بلا مراجعٍ مُسجَّلٍ · و`A-2` `PENDING`)
...
بدأ: 2026-09-02        ينتهي الحجز: 2026-09-09
العائق: —
الخطوةُ التالية: مراجعةٌ — ولا مراجعَ مُسجَّلٌ لهذا النطاقِ و`A-2` `PENDING`: حدٌّ مُعلَنٌ لا يُلتَفُّ عليه
...
بدأ: 2026-09-02        ينتهي الحجز: 2026-09-09
العائق: —
الخطوةُ التالية: مراجعةُ المالكِ — و`A-2` `PENDING` فلا مراجعَ مستقلٌّ اليومَ، وهذا حدٌّ مُعلَنٌ لا يُلتَفُّ عليه
