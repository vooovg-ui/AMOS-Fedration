...
النطاق:
    أحدَ عشرَ سطحًا: ستّةٌ باقيةٌ على تطايرِها المُصرَّحِ في `W-029`، وسطحُ ذاكرةٍ
    مؤقّتةٍ يُعادُ بناؤُه من قاعدةِ البياناتِ، وأربعةٌ دائمةٌ — منها **شاهدا ضبطٍ**
    (سجلُّ التدقيقِ والمهمّةُ) لولاهما لصارَ «فُقِدَ الكلُّ» تفسيرُه أنَّ المِسبارَ
    نفسَه مكسورٌ لا أنَّ الحالةَ تتطايَر.

    ومنها **مفتاحُ الإيقافِ** وطلباتُ الترقيةِ: كانا متطايرَينِ حتّى W-030، ثمّ حُسِمَ
    في `Q-39 (أ)` بقرارِ المالكِ 2026-08-23 أن يُدامَا أوّلًا، فأُديما في `W-031`
    فانتقلَ تصنيفُهما هنا إلى `DURABLE_CONTROL` — والمِسبارُ هو الذي يُثبِتُ ذلك أو
    يُكذِّبُه، لا الترويسة.
    ...
    قرارٌ سياديٌّ — `Q-39` في `docs/audit/SOVEREIGN_DECISION_REGISTER.md`.
...
SURFACES: tuple[Surface, ...] = (
    Surface(
        ...
        "governance",
        DURABLE,
        "لا شيءَ بعدَ W-031: المستوى في جدولِ `system_state` — إيقافٌ لا يرفعُه إقلاعٌ",
        "kill_switch",
        "kill_switch",
    ),
    Surface(
        ...
        "governance",
        DURABLE,
        "لا شيءَ بعدَ W-031: الطلباتُ وبوّاباتُها في جدولِ `promotions` — إذنُ إنسانٍ يبقى",
        "promotion",
        "promotion",
    ),
    Surface(
...
def _write_json(results: list[Result], summary: dict[str, Any]) -> Path:
    ...
    payload = {
        ...
        "note": (
            ...
            "سياديٌّ (Q-39؛ حُسِمَ شقُّه الأوّلُ 2026-08-23 فأُديمَ مفتاحُ الإيقافِ "
            "والترقياتُ في W-031). و`lost_on_restart` **ليس** مخالفةً جديدةً ولا يدخلُ "
