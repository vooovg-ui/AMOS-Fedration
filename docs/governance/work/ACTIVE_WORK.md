...
## تاريخ آخر تعديل: 2026-08-28 (‏`WI-014` — قاعدةُ «لكلِّ قيدٍ مفتوحٍ حرسٌ أو استحقاقٌ» (`DISC-018`) صارَت عدَّادًا يُقاسُ ويُشعِلُ، والخرقُ مقيسٌ بأحدَ عشرَ صفًّا (`DISC-020` · `W-059`) · وقبلَه `WI-013` — شرطُ القياسِ الحيِّ صارَ مقيسًا من مُخرَجِ المِسبارِ · وتعارُضُ `DISC-012` يحبِسُ `WI-006`…`WI-014` مفتوحةً)
