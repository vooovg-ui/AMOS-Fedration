...
## تاريخ آخر تعديل: 2026-08-28 (‏`WI-015` — حرسُ الأسرارِ كانَ يُبلِّغُ عن سرٍّ لا وجودَ له، واختبارُ ما بعدَ الدمجِ كانَ يمرُّ لعلّةٍ غيرِ التي ادّعاها؛ فصارَ الإعفاءُ مُعلَنًا محدودًا والجذرُ مُمَرَّرًا صريحًا)
...
| WI-015 | tooling-gates | T0 (صدقُ القياسِ · إزالةُ أحمرَ كاذبٍ وأخضرَ كاذبٍ) | Driving H | المجلس التأسيسي | IN_REVIEW | tools/crown/verify_secret_boundaries.py · tests/crown/test_secret_boundary_declaration.py · .github/workflows/ci.yml | 2026-08-28 | 2026-09-04 | — | بعدَ دمجِ المالكِ يُنقَلُ إلى `CLOSED` بقيدِ `W-059` ورقمِ الدمجِ — إن فُتِحَ طريقُ الإغلاقِ بقرارِ `A-2` | — (يُكتَبُ بعدَ دمجِ المالك) |
...
### WI-015 — حرسُ الأسرارِ يعرفُ الإعفاءَ المُعلَنَ فيكفُّ عن الإبلاغِ عن سرٍّ لا وجودَ له

```text
النطاق: tooling-gates
المسار/المرحلة: T0 — صدقُ القياسِ: أن لا يُبلِّغَ حرسٌ عن سرٍّ لا وجودَ له، وأن لا يمرَّ اختبارٌ لعلّةٍ غيرِ التي ادّعاها
المالك: Driving H            المراجع: المجلس التأسيسي
الحالة: IN_REVIEW
المسارات:
  tools/crown/verify_secret_boundaries.py                    (DISC-021: `_pem_hits` · `_declared_not_secret_at` · `_is_key_material_at` · `declared_exempt_lines` · وضعُ `--tree-pem-only`)
  .github/workflows/ci.yml                                   (DISC-021: بوّابةُ 6 صارت تستدعي الماسحَ المرجعيَّ، فارتفعَ `grep` مُضمَّنٌ كانَ مصدرَ حقيقةٍ ثانيًا)
  tests/crown/test_secret_boundary_declaration.py            (حرسُ الحرسِ: 8 فحوصٍ · يرفضُ سرًّا حقيقيًّا وإن أُعلِنَ · ولا يُعفي ملفًّا كاملًا ولا شِفرةَ إنتاجٍ)
ومسارَا `DISC-022` **ليست من هذا البندِ**: قُسِمَ البندُ بـ§ 6.1 فبقيَ تصحيحُ
  `check_work_governance.py` و`test_w054_post_merge_reverse_link.py` في `WI-009` — فهما مقفولانِ له وهو `IN_REVIEW`،
  والعطبُ عطبُ تسليمِه (§ 6.5)، والمالكُ واحدٌ؛ فلا `CLAIM_CONFLICT` ولا بندٌ ثانٍ يُنازِعُ نطاقًا مقفولًا
وملفّاتٌ مُعفاةٌ من الحجزِ تُمَسُّ ولا تُدَّعى مِلكًا — مُعلَنٌ لا مطويّ:
  ACTIVE_WORK.md · DISCOVERIES.md · COMPLETION_LEDGER.md · PROJECT_STATE.md · docs/PROJECT_HANDBOOK.md
  و`docs/audit/TRUTH_MATRIX.md` · `docs/audit/truth_matrix.json` — يُعادُ توليدُهما بأمرِ التدقيقِ نفسِه، لا يُحرَّرانِ بيدٍ
  ووثيقتا الحالةِ تُحدَّثانِ لأنَّ `W-059` قُيِّدَ — واجبٌ يحرسُهُ `state_document_drift.py` (قُمْتُهُ صفرٌ قبلَ الدفعِ وبعدَه)
لماذا هذا البندُ الآنَ — سببٌ مُعلَنٌ لا ترتيبٌ مُخترَعٌ:
  - لم يُختَرْ من § 6 اختيارًا: **قِيسَت `ci.yml` أوّلَ مرّةٍ في تاريخِ المشروعِ** (الجولةُ `33126730912` · فرعُ
    `develop` · `e5efe9c`)، فكانت 13 وظيفةً: 9 ناجحةٌ و4 ساقطةٌ، وسببُ السقوطِ كلِّه عطبانِ في القياسِ
    نفسِه لا في المحروسِ — قُيِّدا `DISC-021` و`DISC-022`
  - ورفعَهما المُنفِّذُ قرارًا سياديًّا بخياراتٍ، **فاعتمدَ المالكُ الخيارَ (2) في كلَيهما وأذنَ بالإصلاحِ**
    وحدَّ الإذنَ: لا تعطيلَ بوّابةٍ · لا تخطّيَ فشلٍ · لا تعديلَ مصفوفةٍ بيدٍ · لا تغييرَ تعريفِ `PROVEN`
    · لا تغييرَ خارطةٍ ولا حالةِ بندٍ لتخضيرِ CI · ولا إصلاحَ أكثرَ من هذَينِ العَوارَين
  - وأحمرُ كاذبٌ عن **تسريبِ مفتاحٍ ملكيٍّ** أخطرُ ما يُنسَبُ إلى مستودعٍ: يُهدِرُ التشخيصَ ويُعلِّمُ
    القارئَ تجاهُلَ حرسِ الأسرارِ. وأخضرُ كاذبٌ في `W-054` أخطرُ منه: بوّابةٌ تبدو محروسةً وهي غيرُ محروسةٍ
  - ولا يلزمُه قرارٌ سياديٌّ بعدَ إذنِ المالكِ، ولا سرٌّ ولا حسابُ Actions جديدٌ — فلا يقعُ تحتَ عائقٍ مفتوحٍ
  - والمساراتُ الخمسةُ **غيرُ محجوزةٍ لبندٍ مفتوحٍ**: `WI-009` يحجزُ `check_work_governance.py`
    و`test_w054_post_merge_reverse_link.py` وهو `IN_REVIEW` — والحجزُ يُرفَعُ عندَ الدمجِ (§ 6)، وقد
    دُمِجَ قيدُه `W-054` في `origin/main`؛ فالمسُّ هنا **امتدادٌ مُعلَنٌ لعطبٍ قِيسَ بعدَ الدمجِ** لا حجزٌ مُنازَعٌ
خارجَ النطاق:
  - **لا تُوسَّعُ العلامةُ ولا تُخترَعُ ثانيةٌ**: العلامةُ هي `truth-audit: not-a-secret` عينُها التي يعرفُها
    `tools/governance/truth_audit.py` — مصدرُ حقيقةٍ واحدٌ لمعنى «ليست سرًّا»، وحرسٌ يُثبِتُ ذلك نصًّا
  - لا يُلمَسُ `resolve_merge_base`: حدُّها «ومرجعٌ مُمَرَّرٌ صراحةً لا يُتجاوَزُ إلى غيرِه» **حدٌّ صحيحٌ**،
    وتغييرُه لتخضيرِ اختبارٍ إفسادٌ للمقيسِ إرضاءً لقارئِه
  - لا تُخفَّفُ بوّابةٌ ولا يُضيَّقُ نطاقُها: بوّابةُ 6 تمسحُ شجرةَ العملِ كما كانت، والماسحُ المرجعيُّ
    **أقوى** من `grep` (يقرأُ الثنائيَّ ويُعلِنُ ما تعذّرت قراءتُه)
  - لا يُحذَفُ فحصٌ ولا يُضعَفُ تأكيدٌ ولا يُبدَّلُ معيارُ قبولٍ: عددُ التأكيداتِ زادَ ولم ينقُصْ
  - لا تُعالَجُ الوظيفتانِ الساقطتانِ الأُخرَيانِ في تلك الجولةِ إن كانَ سببُهما غيرَ هذَينِ العَوارَينِ:
    يُقاسُ الأثرُ بإعادةِ التشغيلِ، وما بقيَ يُقيَّدُ `DISC` جديدةً ولا يُتجاوَز
  - لا يُبدَأُ `WI-014`: تسجيلُ `A-2`/`A-3` وإغلاقُ `WI-006`…`WI-013` **موقوفٌ بنصِّ إذنِ المالكِ**،
    والمعرِّفُ `WI-014` يُترَكُ محجوزًا له فلا يُشغَلُ بغيرِه
  - لا يُدمَجُ هذا العملُ بيدِ المُنفِّذِ، ولا يُدَّعى `PROVEN` لأيِّ سطحٍ من سطوحِ المصفوفةِ
معيارُ القبول:
  1. حارسُ الأسرارِ **يرفضُ مادةَ مفتاحٍ صالحةً وإن حملَ موضعُها العلامةَ** — بدايةٌ وجسدُ base64 وخاتمةٌ
  2. ولا يُعَدُّ سرًّا موضعٌ **مُعلَنٌ صراحةً** في `tests/` ليس فيه مادةُ مفتاحٍ
  3. الإعفاءُ لا يمتدُّ: سطرٌ خالٍ يقطعُ الفقرةَ · ولا إعفاءَ خارجَ `tests/` · ولا إعفاءَ في التاريخِ
     إلّا بإعلانٍ **حيٍّ في الشجرةِ** (فمتى حُذِفَ الإعلانُ عادَ السطرُ مخالفًا)
  4. الإعفاءاتُ **تُعَدُّ وتُعلَنُ** في دليلِ البوّابةِ، فلا يُخفى عددُها
  5. بوّابةُ 6 في `ci.yml` تستدعي الماسحَ المرجعيَّ وحدَه — لا ماسحَ ثانيًا في المشروع
  6. اختبارُ `W-054` **يُمرِّرُ الجذرَ صراحةً** في كلِّ عمليّةٍ فرعيّةٍ، فلا يقرأُ المستودعَ الحقيقيَّ
  7. الإسقاطُ يُطلَبُ فيسقُطُ برمزِ 1 و`POST_MERGE_NOT_CLOSED` **يُسمّي بندَ شجرةِ الاختبارِ** (`WI-006`)
  8. والإغلاقُ الحقيقيُّ يُمرِّرُه برمزِ 0 — فلا تُقاسُ الحمرةُ وحدَها
  9. شجرتانِ لا تختلفانِ إلّا في الإغلاقِ تُعطيانِ حكمَينِ مختلفَينِ · والحكمُ لا يتغيَّرُ بموضعِ التشغيلِ
 10. `--repo-root` **ليسَ رايةَ تخطٍّ**: الفحوصُ عينُها تُجرَى كاملةً، ومسارٌ ليسَ مجلَّدًا يُرفَضُ برمزِ 2
 11. الانحدارُ المحلّيُّ لم ينقُصْ: 2094 مارًّا (‏+12 فحصًا جديدًا) · 0 ساقطًا · والأخطاءُ الثلاثةُ نفسُها من `DISC-009`
 12. بوّاباتُ الفحصِ الثلاثُ لـruff عندَ نُسخةِ CI المُثبَّتةِ (`0.6.9`) نظيفةٌ · والمصفوفةُ أُعيدَ توليدُها ولم تُحرَّرْ بيدٍ
الدليلُ المطلوب:
  pytest tests/crown/test_secret_boundary_declaration.py -q                      # 8 passed
  pytest tests/governance/test_w054_post_merge_reverse_link.py -q                # 15 passed
  python tools/crown/verify_secret_boundaries.py --tree-pem-only                 # PASS 1/1 · إعفاءٌ واحدٌ مَعدود
  pytest tests/ -q                                                              # 2094 passed · 3 skipped · 3 errors (DISC-009)
  ruff check .  ·  ruff check src/ tests/  ·  ruff format --check src/ tests/    # ruff==0.6.9 · نظيفةٌ ثلاثتُها
...
حدُّ البندِ — مُعلَنٌ لا مطويٌّ:
  هذا البندُ يُثبِتُ أنَّ حرسَ الأسرارِ صارَ يفرِّقُ بينَ سرٍّ ومَذكَرِ نمطِه في اختبارٍ سلبيٍّ مُعلَنٍ،
  وأنَّ حرسَ ما بعدَ الدمجِ صارَ يُقاسُ على الشجرةِ التي بناها الاختبارُ لا على المستودعِ الحقيقيِّ.
  **ولا يُثبِتُ أنَّ CI خضراءُ**: بقيّةُ أسبابِ السقوطِ تُقاسُ بإعادةِ تشغيلِ `ci.yml` بعدَ الدفعِ، وما
  ظهرَ جديدًا يُقيَّدُ `DISC`. **ولا يُثبِتُ أنَّ لا سرَّ في المستودعِ أبدًا**: الإعفاءُ يُوسِّعُ ما يُقبَلُ
  في `tests/` بثلاثةِ قيودٍ مجتمعةٍ، وسطرٌ واحدٌ يحملُ بدايةَ PEM بلا جسدٍ يمرُّ إن أُعلِنَ — والحدُّ
  المقيسُ أنَّ **مادةَ المفتاحِ لا تمرُّ ولو أُعلِنَت**. ومحلُّ خطرِه المكتوبُ `DISC-021` و`DISC-022`.
```
...
تصحيحٌ داخلَ البندِ — `DISC-022` (‏2026-08-28 · بإذنِ المالكِ صريحًا · § 6.5):
  قِيسَت `ci.yml` أوّلَ مرّةٍ (الجولةُ `33126730912`) فسقطَ فحصٌ واحدٌ من حرسِ هذا البندِ:
  `test_الإسقاطُ_يُطلَبُ_فيسقُط`. والعلّةُ فيه لا في المحروسِ: كانَ يبني شجرةً في `tmp_path`
  ويُرقِّعُ `gate.REPO_ROOT` بها ثمَّ يُشغِّلُ الأداةَ **عمليّةً فرعيّةً** — والترقيعُ لا يبلُغُ
  عمليّةً أخرى — فتشتقُّ الأداةُ جذرَها من موقعِ ملفِّها فتقيسُ **المستودعَ الحقيقيَّ**. فمرَّ
  محلّيًّا برمزِ 1 آتٍ من `POST_MERGE_NOT_CLOSED` **حقيقيٍّ في المستودعِ** — أي **لعلّةٍ غيرِ
  التي ادّعاها** — وسقطَ في التكاملِ حيثُ لا فرعَ `main` محلّيًّا بل `origin/main` وحدَه.
  والإصلاحُ أضيقُ ما يكفي: رايةٌ صريحةٌ `--repo-root PATH` (و`set_repo_root`) تُعيِّنُ محلَّ
  القياسِ فلا يُخمَّن — والفحوصُ عينُها تُجرَى كاملةً، فليست رايةَ تخطٍّ — ويُمرِّرُها الاختبارُ
  في كلِّ عمليّةٍ فرعيّةٍ. و**`resolve_merge_base` لم تُمَسّ**: حدُّها «ومرجعٌ مُمَرَّرٌ صراحةً
  لا يُتجاوَزُ إلى غيرِه» حدٌّ صحيحٌ، وتغييرُه لتخضيرِ اختبارٍ إفسادٌ للمقيسِ إرضاءً لقارئِه.
  وزادَ الحرسُ أربعةَ فحوصٍ (11 ← 15): الإسقاطُ يسقُطُ ويُسمّي بندَ شجرةِ الاختبارِ · والإغلاقُ
  الحقيقيُّ يُمرِّرُه برمزِ 0 · وشجرتانِ لا تختلفانِ إلّا في الإغلاقِ تُعطيانِ حكمَينِ مختلفَينِ
  · والحكمُ لا يتغيَّرُ بموضعِ التشغيلِ · وغيابُ الأساسِ يُعلَنُ ولا يُطوَى.
  الدليلُ: مُحاكاةُ ظرفِ التكاملِ (نسخةُ شجرةٍ · `HEAD` منفصلٌ · لا `main` محلّيٌّ): **قبلَ**
  الإصلاحِ `1 failed, 10 passed` و**بعدَه** `15 passed`. وقيدُه في السجلِّ `W-059`، ومحلُّ
  خطرِه `DISC-022`. **وحالةُ البندِ لم تُغيَّرْ**: يبقى `IN_REVIEW` وإغلاقُه محجوبٌ بـ`A-2`.
خارجَ النطاق:
