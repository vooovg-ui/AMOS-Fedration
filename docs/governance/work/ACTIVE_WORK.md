...
| WI-035 | tooling-gates | T0 (‏حدُّ الصدقِ 9 في [`COMPLETION_LEDGER.md § 10`](../../audit/COMPLETION_LEDGER.md): صدقُ طوابعِ الالتزامِ يصيرُ رقمًا مقيسًا محروسًا من النموِّ · `DISC-043`) | Driving H | مجلس المراجعة (GPT 5.6 + Grok 4.6) | IN_PROGRESS | tools/governance/commit_timestamp_integrity.py · tests/governance/test_w110_commit_stamp_integrity.py · tests/governance/test_w113_new_commit_stamp_utc.py · tests/governance/test_w164_platform_merge_scope.py | 2026-09-02 | 2026-09-30 | **وجُدِّدَ الحجزُ إلى 2026-09-30 بنصِّ § 6.3** — والسببُ مكتوبٌ لا مطويٌّ: البندُ لم يُسلَّمْ ولم يُشغَّلْ له مراجعٌ بعدُ، والتجديدُ **رفعُ حاجزٍ زمنيٍّ** لا إنجازٌ ولا مراجعةٌ **وقد شُغِّلَت الجولةُ الخامسةُ من مجلسِ المراجعةِ المستقلِّ (`Q-43`) على عقدةِ `b951dcac6cd103d396e8daa4e5b9e4cefb3f9c5a` يومَ 2026-09-13، فبطلَ مضمونُ العبارةِ السابقةِ «لم يُشغَّلْ له مراجعٌ بعدُ» — وتبقى مكتوبةً شاهدةً على تاريخِها لا تُمحى.** والحكمُ: **`REJECTED` بانقسامٍ لا يُجمَعُ جمعًا حسابيًّا** (أ `VERIFIED` `P1=0` · ب `REJECTED` `P1=2`) — والعيوبُ مُقيَّدةٌ باتّحادِها في `DISC-063` **ولم تُصلَحْ ههنا** (§ 5.2)، فالعائقُ الآنَ: معالجةُ عيوبِ الجولةِ الخامسةِ ثمَّ إعادةُ التسليمِ لجولةٍ سادسةٍ. **وفي `W-164` قُطِعَ سببُ `DISC-065` في مصدرِه بحصرِ مدى القياسِ في تاريخِ المستودعِ — بإذنِ المالكِ نصًّا، وبلا رفعِ رقمٍ مُعلَنٍ ولا تخفيفِ شرطٍ ولا مسِّ `ci.yml`.** | معالجةُ عيوبِ الجولةِ الخامسةِ المُقيَّدةِ في `DISC-063` بيدِ مالكِ البندِ، ثمَّ إعادةُ التسليمِ `IN_PROGRESS → IN_REVIEW` لجولةٍ سادسةٍ — و`VERIFIED` فعلُ المجلسِ لا فعلُ المؤلِّفِ (§ 4.3 · `Q-43`) | — |
...
| WI-052 | audit-truth | T0 (‏قابليّةُ القياسِ · أداءُ واجبِ § 7/6 على عقدةِ `W-158` ثمَّ تشغيلُ الجولةِ الخامسةِ من مجلسِ المراجعةِ المستقلِّ (`Q-43`) على البنودِ الستّةِ التي تقفلُ مساراتِ بنودِ `READY` الخمسةِ) | Perplexity Computer | مجلس المراجعة (GPT 5.6 + Grok 4.6) | IN_REVIEW | docs/audit/COMPLETION_LEDGER.md · docs/governance/work/ACTIVE_WORK.md | 2026-09-13 | 2026-09-30 | لا عائقَ فنيًّا على هذا البندِ — ولا مسارَ مقفولًا يُمَسُّ. **والعائقُ البنيويُّ المقيسُ الذي أنشأَ هذا البندَ**: عشرونَ بندًا `IN_REVIEW` تقفلُ مساراتِها (§ 6.1) فتحجُبُ بنودَ `READY` الخمسةَ، والمراجعةُ فعلُ المجلسِ لا فعلُ المؤلِّفِ (§ 4.3) **والجولةُ الخامسةُ شُغِّلَت وقُيِّدَ حكمُها في `W-161`: صفرُ بندٍ مُجمَعٌ عليه ⇒ صفرُ `VERIFIED`، فنُقِلَت الستّةُ `IN_REVIEW → IN_PROGRESS` — والعائقُ البنيويُّ المقيسُ لم يُرفَعْ: المساراتُ تبقى مقفولةً في `IN_PROGRESS` كما في `IN_REVIEW` (§ 6.1)، وبنودُ `READY` الخمسةُ تبقى محجوبةً.** **وحكمُ عقدةِ `W-161` على `main` سقطَ بمُضِيِّ اليومِ فعولِجَ سببُه الجذريُّ بتجديدٍ مُعلَّلٍ في `W-162` بلا محوٍ ولا تخفيفٍ.** **وحكمُ عقدةِ `W-162` قُرِئَ أخضرَ 13/13 بساعةِ `2026-09-14T14:56:41Z` فاستُوفِيَ معيارُ 9، فنُقِلَ البندُ `IN_PROGRESS → IN_REVIEW` (`W-163`).** | مراجعةُ مجلسِ المراجعةِ (‏`Q-43`) في جولةٍ سادسةٍ — و`VERIFIED` فعلُ المجلسِ لا فعلُ المؤلِّفِ (§ 4.3)، ولا يُنقَلُ بندٌ بلا إجماعٍ | — (‏يكتبُها مَن يجبُ عليه الإغلاقُ · `DISC-010` · `DISC-030`: ذِكرُ قيدٍ أُضيفَ في مجموعةِ التغييرِ نفسِها يُشعِلُ `POST_MERGE_NOT_CLOSED` على بندٍ غيرِ مُغلَقٍ — والدليلُ لم يُخفَ: القيدُ مُسمًّى بعينِه ورقمِه في كتلةِ التفاصيلِ أدناه، وصفُّه في § 8 يُسمّي هذا البندَ نصًّا) |
...
المسارات:
  ...
  tests/governance/test_w164_platform_merge_scope.py               (‏جديدٌ `W-164` — نطاقُ القياسِ: عقدةُ المنصّةِ تُستثنى وتُعلَنُ، والخرقُ الحقيقيُّ يبقى مقيسًا · `DISC-065`)
فحصُ التداخلِ (§ 6.2) — نتيجتُه تُقيَّدُ ولو كانت «لا تداخل»:
  ...
  - وملفُّ `test_w164_platform_merge_scope.py` **جديدٌ ومُعدَّدٌ اسمًا** في خليّةِ مساراتِ هذا البندِ (‏لا نثرًا · درسُ `DISC-042`): قِيسَ على خلايا المساراتِ كلِّها — لا تداخل
...
مدى القياسِ — إصلاحُ W-164 (‏سببُ DISC-065 لا عَرَضُه):
  الحرسانِ كانا يقيسانِ `git log HEAD`، و`HEAD` في فحصِ طلبِ الدمجِ عقدةُ دمجٍ
  عارضةٌ يُولِّدُها GitHub في refs/pull/<N>/merge بإزاحةِ ساعةِ المُولِّدِ — فليست
  من تاريخِ المستودعِ ولن تدخلَه. فصارَ الحكمُ على pull_request غيرَ حتميٍّ:
  الشجرةُ واحدةٌ والحكمُ يتبدَّلُ (تشغيلُ 28 أخضرُ 13/13 · وتشغيلا 30 و32 أحمرانِ).
  والعلاجُ حصرُ المدى لا تخفيفُ حدٍّ: platform_merge_head() بشرطٍ ثلاثيٍّ مقيسٍ،
  ويُقاسُ كلُّ تاريخِ الأبوَينِ بدلَ العقدةِ فلا ينخفضُ دَينٌ (413 التزامًا لا 412)،
  والاستثناءُ يُعلَنُ في خرجِ الأداةِ باسمِ العقدةِ وسببِه فلا يمرُّ صامتًا.
  والقياسُ قبلَ الإصلاحِ وبعدَه مكتوبٌ في صفِّ DISC-065: 136≠135 رمزُ 1 ⇒ 135=135 رمزُ 0.
  وأربعةُ فحوصٍ تُثبِتُ أنَّ الاستثناءَ ليس بابًا: خرقٌ على فرعٍ يبقى مقيسًا،
  ودمجٌ يحويه فرعٌ يُقاسُ ولو حملَ إزاحةً محلّيّةً.
  وهذا الإصلاحُ **لا يُغلِقُ البندَ ولا DISC-065**: بقيَّةُ عيوبِ الجولةِ الخامسةِ
  (DISC-063) لم تُعالَجْ ههنا، والقبولُ فعلُ المجلسِ لا فعلُ المؤلِّفِ.
...
الحالة: IN_REVIEW   (‏`IN_PROGRESS → IN_REVIEW` · § 4.3 — سُلِّمَ بعدَ استيفاءِ المعاييرِ
  التسعةِ وقراءةِ حكمِ CI على عقدةِ كلِّ قيدٍ؛ ولا يكتبُ المؤلِّفُ VERIFIED)
المسارات:
  docs/audit/COMPLETION_LEDGER.md            (معفيةٌ من الحجزِ بنصِّ § 6)
  docs/governance/work/ACTIVE_WORK.md        (معفيةٌ من الحجزِ بنصِّ § 6)
...
الخطوةُ التالية: مراجعةُ مجلسِ المراجعةِ (Q-43) في جولةٍ سادسةٍ — والنقلُ إلى
  VERIFIED فعلُ المجلسِ لا فعلُ المؤلِّفِ، ولا يُنقَلُ بندٌ بلا إجماعٍ
...
ما اكتُشِفَ بعدَ دمجِ W-161 — مُقيَّدٌ لا مطويٌّ (قيدُه W-162):
  حكمُ CI على عقدةِ W-161 (80b6324) قُرِئَ أخضرَ 13/13 في تشغيلِ 28 (pull_request)
  ثمَّ أحمرَ 12/1/0 في تشغيلِ 29 (push · main) على **عينِ الشجرةِ** بفارقِ إحدى
  وعشرينَ دقيقةً، بمخالفتَي DUE_DATE_PASSED لـDISC-055 وDISC-056 اللذَينِ مضى
  استحقاقُهما بدخولِ 2026-09-14. فالفرقُ الساعةُ لا الشجرةُ — ثالثُ دليلٍ حيٍّ
  على DISC-062. وعولِجَ السببُ في محلِّه: تجديدٌ إلى 2026-09-30 بسببٍ مكتوبٍ في
  صفِّ كلٍّ منهما بلا محوِ الأوّلِ، ولا بوّابةَ خُفِّفَت ولا فحصَ أُسكِتَ.

حكمُ CI على عقدةِ القيدِ الأخيرِ — واجبُ § 7/6 (قيدُه W-163):
  العقدةُ المدموجةُ 0550d9a · تشغيلُ 34856421846 (رقمُ 31 · محاولةُ 1 · push · main)
  ⇒ success · 13 خضراءَ · 0 حمراءَ · 0 متخطًّى، ومصفوفةُ الحقيقةِ 34856421954 ⇒ 1 خضراءُ.
  ساعةُ القراءةِ 2026-09-14T14:56:41Z (DISC-062). فاستُوفِيَ معيارُ القبولِ 9 ونُقِلَ
  البندُ إلى IN_REVIEW. وحمرةُ تشغيلِ 30 (pull_request) مُقيَّدةٌ في السجلِّ بسببِها
  المقيسِ (DISC-065) ولم تُمحَ ولم تُبرَّرْ بخُضرةٍ محلّيّةٍ.
