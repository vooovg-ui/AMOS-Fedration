...
def test_sub_scale_cost_was_recorded_as_zero_until_q42_widened_the_scale() -> None:
    """حدُّ تمثيلِ المالِ مقيسٌ لا مسكوتٌ عنه — وقد **حُسِمَ** في Q-42 فتبدَّلَ الرقمُ.

    ## ما كانَ (حتى 2026-08-24)
    ...
    وكانَ هذا الفحصُ يُثبِّتُ الصفرَ ويقولُ: «إن غُيِّرَ المقياسُ سقطَ الفحصُ ولزمَ
    قرارٌ مكتوبٌ — لا انحرافٌ صامتٌ».

    ## وقد سقطَ الفحصُ بالقرارِ لا بالانحرافِ — وهذا هو النصُّ الجديد

    حسمَ المالكُ في **Q-42 · الشقِّ الثاني · الخيارِ (أ)** بتاريخِ 2026-08-24:
    «**توسيعُ دقّةِ أعمدةِ الكلفةِ وحدَها**». فصارَ المقياسُ ثمانِ منازلَ
    (`COST_SCALE`) في أعمدةِ الكلفةِ دونَ سائرِ المالِ (هجرةُ 016 · `W-036`).

    فالقيمُ المُثبَّتةُ أدناهُ **تغيَّرَت بقرارٍ مكتوبٍ لا بدَريفٍ صامتٍ**، والنصُّ
    القديمُ محفوظٌ أعلاهُ كما كانَ: الوثيقةُ تُصحَّحُ بالقياسِ ولا يُمحى تاريخُها.
    والشرطُ نفسُه باقٍ سارِيًا على الحدِّ الجديدِ: من ضيَّقَ أو وسَّعَ ثمانَ المنازلِ
    مرّةً أخرى أسقطَ هذا الفحصَ ولزمَه قرارٌ مكتوبٌ.
    """
    convert = model_layer_module._money_from_provider_float
    assert (
        str(convert(0.00003)) == "0.00003000"
    ), "تغيَّرَ سلوكُ تمثيلِ الكلفةِ بلا قرارٍ مكتوبٍ — أو أنَّ Q-42 (أ) لم يُنفَّذْ"
    assert str(convert(0.0003)) == "0.00030000"
    assert str(convert(7.77)) == "7.77000000"

    # والعيبُ الذي رُفِعَ يُقاسُ لا يُوصَفُ: هذا هو الجوابُ القديمُ بعينِه.
    assert (
        str(to_money(f"{0.00003:.4f}")) == "0.0000"
    ), "لو لم يكن العقدُ القديمُ يُبلِعُ الكسرَ لم يكن في Q-42 موضوعٌ"
    assert convert(0.00003) > 0, "بعدَ التوسيعِ لا يجوزُ أن تبقى كلفةٌ موجبةٌ صفرًا"
