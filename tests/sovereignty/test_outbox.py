...
def _ترويسةُ_مفتاحٍ_مُركَّبةٌ() -> str:
    """ترويسةُ PEM تُبنى بالتركيبِ لا تُكتَبُ حرفًا — والقيمةُ في الاختبارِ كاملةٌ.

    القيمةُ التي يراها الحرسُ ترويسةُ مفتاحٍ خاصٍّ تامّةٌ، فالفحصُ لم يَضعُفْ ولم
    يُستبدَلْ بشيءٍ أهونَ. والمكتوبُ في نصِّ المِلَفِّ أجزاءٌ لا كتلةٌ، كي لا يحملَ
    مستودعٌ منشورٌ ما يقرأُه ماسحُ الأسرارِ مادّةَ مفتاحٍ — وهي عينُ الحيلةِ التي
    يستعملُها tools/crown/verify_secret_boundaries.py لنفسِه في بناءِ نمطِه.
    والسببُ مقيسٌ لا متوقَّعٌ: الكتلةُ المكتوبةُ حرفًا هنا أسقطَت بوّابتَينِ في CI
    عندَ كلِّ تشغيلٍ على main — انظر DISC-024 والقيدَ W-064.
    """
    رأسٌ = "".join(("BEG" + "IN", " ", "PRIV" + "ATE", " ", "KEY"))
    return ("-" * 5) + رأسٌ + ("-" * 5) + "abc"
...
class Testسلامةُالحمولة:
    ...
    @pytest.mark.parametrize(
        "قيمة",
        [
            pytest.param(_ترويسةُ_مفتاحٍ_مُركَّبةٌ(), id="ترويسةُ-مفتاحٍ-مُركَّبةٌ"),
